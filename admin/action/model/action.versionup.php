        ...
        for ($i=0;$i>$diff;$i--){
            ...
            foreach (Config_Game::$game as $gameid=>$gamename){
                foreach (Config_Game::$clientTyle as $ctype=>$clientname){
                    $userNum    = Loader_Redis::common()->hGetAll("versionupUsernum|$gameid|$ctype|$time");
                    $userCi     = Loader_Redis::common()->get("versionupUserci|$gameid|$ctype|$time", false, false);
                    $canyuNum   = Loader_Redis::common()->hGetAll("versionupCanyunum|$gameid|$ctype|$time");
                    $canyuCi    = Loader_Redis::common()->get("versionupCanyuci|$gameid|$ctype|$time", false, false);
                    $FF         = Loader_Redis::common()->get("versionupFafang|$gameid|$ctype|$time", false, false);
