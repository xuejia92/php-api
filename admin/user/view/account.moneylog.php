...
function autoclick(){
	...
	var m_table = $("#m_table").val();
	var gid = $("#m_gameid").val();
	$("#a_moneylog").attr("href","?m=user&p=account&act=moneylog&stime="+stiem+"&etime="+etiem+"&mid="+mid+"&table="+m_table+"&gid="+gid) ;
	$("#a_moneylog").click(); 
}

</script>

<form rel="momeylog" id="pagerForm" method="post" action="?m=user&p=account&act=moneylog">
...
<div class="pageHeader" >
	<!--  <form  rel="moneylog" class="pageForm required-validate"  onsubmit="return iframeCallback(this, navTabAjaxDone)" action="?m=user&p=account&act=moneylog" method="post">  --> 
	<div class="searchBar">
		<table class="searchContent">
			<tr>
				...
				<td>表：</td>	
				<td>					
					<select id="m_table" style="float:left" name="table">
...
<div class="pageContent">
	<table class="table" width="100%" layoutH="138">
		...
		<?php foreach($items as $item):?>
			<tr target="id" rel="<?php echo $item['id']?>">				
				<td><?php echo date("Y-m-d H:i:s",$item['wtime']) ?></td>
				<td><?php echo $item['wflag'] == 0 ? "收获" :"付出" ?></td>
				<td><?php echo $item['money'] ?></td>
				<td><?php echo Config_Game::$game[$item['gameid']] ?></td>
				<td><?php echo $item['desc'].'&nbsp;&nbsp;'.$item['wdesc'] ?></td>				
			</tr>
		<?php endforeach;?>	
		</tbody>
	</table>
	<div  class="panelBar">
		...
		<div rel="moneylog" class="pagination" targetType="navTab" totalCount="<?php echo $totalCount?>" numPerPage="<?php echo $numPerPage?>" pageNumShown="10" currentPage="<?php echo $currentPage?>"></div>
	</div>
</div>
