...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="?m=user&p=rank&act=getDeviceRank&navTabId=devicerank&type=<?php echo $_REQUEST['type']?>" method="post">
	<div class="searchBar">
		...
		<?php foreach($items as $k=>$item):?>
			<tr target="id" rel="<?php echo $item['id']?>">	
				...
				<td>  <a class="button"  height="800" width="800" href="?m=user&p=account&act=getdevices&deviceno=<?php echo $item['deviceno'] ?>" target="dialog" rel="sdf" title="查看与此机器码相同的用户"><span><?php  echo  $item['deviceno']?></span></a>
					<?php if($item['dflag']):?>
					&nbsp;&nbsp;&nbsp;已被封&nbsp;&nbsp;&nbsp;
					<a  class="delete" href="?m=user&p=account&act=resetDeviceNo&deviceno=<?php echo $item['deviceno']?>&navTabId=devicerank" target="ajaxTodo" title="你确定要解封？" >
					解封
					</a>
					<?php else:?>
					<a style='color:red;float:right' class="delete" href="?m=user&p=account&act=banDeviceNo&navTabId=devicerank&deviceno=<?php echo $item['deviceno']?>&type=<?php echo $_REQUEST['type']?>" target="ajaxTodo" title="你确定要封这个机器码？" >
					封机器码
					</a>
					<?php endif;?>   
				</td>
				<td><a class="button"  height="800" width="800" href="?m=user&p=account&act=getips&ip=<?php echo $item['ip'] ?>" target="dialog" rel="sdf" title="查看与此IP相同的用户"><span><?php  echo  $item['ip']?></span></a>
					 <?php if($item['ipflag']):?>
					 	&nbsp;&nbsp;&nbsp;已被封&nbsp;&nbsp;&nbsp;
					 	<a  class="delete" href="?m=user&p=account&act=resetbanIp&navTabId=devicerank&ip=<?php echo $item['ip']?>" target="ajaxTodo" title="你确定解封这个IP码？" >
						解封IP 
					 	</a>
					 <?php else:?>
					 <a style='color:red;float:right' class="delete" href="?m=user&p=account&navTabId=devicerank&act=banIP&ip=<?php echo $item['ip']?>" target="ajaxTodo" title="你确定要封这个IP码？" >
						封IP 
					 </a>
					 <?php endif;?>  
