...
class Lib_Mysql
{
    ...
    public function connect(){
        ...
        if ($this->persist){
            $this->conn = @mysql_pconnect($this->host, $this->user, $this->password) or $this->errorlog("connect mysql failed host:{$this->host} user:{$this->user} pass:{$this->password}");
        }else{
        	//$this->conn = @mysql_connect($this->host, $this->user, $this->password) or $this->errorlog("連不上數據庫 host:{$this->host}  user:{$this->user} pass:{$this->password}");
        	$__stime = microtime(true);
        	$this->conn = @mysql_connect($this->host, $this->user, $this->password);
        	if(!$this->conn ){//连接不上，再重新连接一次
        	...
        	if(!$this->conn ){
	            ...
	            Logs::factory()->debug(array($this->host,$this->user,$error,$errno,$etime-$stime,$__etime-$__stime,$_POST['api']),'mysql_connect_error');
	            return false;
        	}
