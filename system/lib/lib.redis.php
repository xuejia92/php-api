...
class Lib_Redis
{
     ...
     * @return Mixed/false
     */
    public function blPop($arrkey,$timeout, $zip=false, $serial=true){
        if( ! $this->connect()){
            return false;
        }
        $value = $this->oRedis->blPop( $arrkey,$timeout);
        $value = $zip ? @gzuncompress( $value) : $value;
        
        $value[1] && $value[1] = $serial ? @unserialize( $value[1] ) : $value[1];
        return $value;
    }
     ...
     * @return Mixed/false
     */
    public function brPop($arrkey, $timeout, $zip=false, $serial=true){
        if( ! $this->connect()){
            return false;
        }
        $value = $this->oRedis->brPop( $arrkey,$timeout);
        $value = $zip ? @gzuncompress( $value) : $value;
